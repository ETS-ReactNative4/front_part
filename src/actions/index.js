...
export const gettingNote = () => {
    const getNotes = axios.get('https://back-end-project-notes.herokuapp.com/notes');
    return dispatch => {
...
export const addingNote = (newNote) => {
    const promise = axios.post('https://back-end-project-notes.herokuapp.com/notes', newNote);
    return dispatch => {
        dispatch({ type: FETCHING_NOTES });
...
export const displayingNote = (_id) => {
    const promise = axios.get(`https://back-end-project-notes.herokuapp.com/notes/${_id}`);
    return dispatch => {
        dispatch({ type: FETCHING_NOTES });
        promise.then(response => {
...
export const deletingNote = (_id) => {
    const promise = axios.delete(`https://back-end-project-notes.herokuapp.com/notes/${_id}`);
    return dispatch => {
        dispatch({ type: FETCHING_NOTES });
        promise.then(response => {
...
export const editingNote = (updatedNote) => {
    ...
    const promise = axios.put(`https://back-end-project-notes.herokuapp.com/notes/${updatedNote.id}`, updatedNote);
    return dispatch => {
        dispatch({ type: FETCHING_NOTES });
        promise.then(response => {
            console.log('response data in deleting note: ', response.data)
